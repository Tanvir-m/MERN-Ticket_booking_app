...
const ContextCom = (props) => {
  ...
  const [seatPriceAndRange, setSeatPriceAndRange] = useState([]);
  ...
  useEffect(() => {
    ...
    // for get seat price and range
    const getSeatPriceAndRange = async () => {
      try {
        const res = await axios.get('/api/admin/get');
        setSeatPriceAndRange(res.data);
      } catch (err) {
        console.log('error in getting getSatPriceAndRange', err);
      }
    };
    getSeatPriceAndRange();

    // for set local storage
